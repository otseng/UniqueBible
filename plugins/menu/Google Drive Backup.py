...
def uploadFiles():
    try:
        noteFileCloudId = os.path.join("plugins", "menu", "GoogleDriveUtility", "noteFileGoogleCloudId.txt")
        upload = subprocess.Popen("{0} {1} upload {2}".format(sys.executable, os.path.join("plugins", "menu", "GoogleDriveUtility", "access_google_drive.py"), config.marvelData), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
